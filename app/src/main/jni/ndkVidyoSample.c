...
VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_SET_OFFLINE, &inEventSetOffline, sizeof(inEventSetOffline));
return;
}
...
JNIEXPORT void JNICALL Java_com_elimnet_kcgvc_VidyoSampleApplication_DisableAutoLogin(JNIEnv *env, jobject jobj)
{
    ...
    LOGE("Enable Auto Login Set, %d",requestConfig.enableAutoLogIn);
	VidyoClientSendRequest(VIDYO_CLIENT_REQUEST_SET_CONFIGURATION, &requestConfig, sizeof(VidyoClientRequestConfiguration));
	//FUNCTION_EXIT
}
...
LOGI("MuteSpeak %d", muteSpeak);
LOGE("MuteSpeak called\n");
VidyoClientInEventMute vMute;
vMute.willMute = muteSpeak;
ret = VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_MUTE_AUDIO_OUT, &vMute,
                           sizeof(VidyoClientInEventMute));
if(ret == VIDYO_TRUE)
LOGE("MuteSpeak successed\n");
else
LOGE("MuteSpeak failed\n");
return;
}
...
//FUNCTION ENTRY
VidyoBool ret = VIDYO_FALSE;
LOGI("MuteMic %d", muteMic);
LOGE("MuteMic called\n");
VidyoClientInEventMute vMute;
vMute.willMute = muteMic;
ret = VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_MUTE_AUDIO_IN, &vMute,
                           sizeof(VidyoClientInEventMute));
//FUNCTION EXIT
if(ret == VIDYO_TRUE)
LOGE("MuteMic successed\n");
else
LOGE("MuteMic failed\n");
return;
}
...
{
strcpy(event.fontFileName, fontFileC);
ret = VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_SET_FONT_FILE, &event, sizeof(VidyoClientInEventSetFontFile));
if(ret == VIDYO_TRUE)
LOGE("setfontfile successed\n");
else
LOGE("setfontfile failed\n");
